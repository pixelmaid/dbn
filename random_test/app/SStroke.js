...
define(['paper', 'app/SignalProcessUtils', 'app/Path'], function(paper, Utils, Path) {
	...
	function SStroke(prototype, spine) {
		...
		if (!prototype) {
			...
			prototype = new paper.Path.Rectangle(new paper.Point(0, 0), new paper.Point(100,5));
		...
		var datapoints = this.ribs.map(function(r) {
			return {
				x: r.is[0].point.x,
				y: r.is[0].point.y
			};
		...
		this.inflectionPoints = utils.calculateInflectionPoints(datapoints);
